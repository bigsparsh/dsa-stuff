...
class Student {
    ...
    int marks;
  public:
    Student(int age, string name, int enroll_no, int marks) : age(age), name(name), enroll_no(enroll_no), marks(marks) {}
    void display() {
...
class Faculty {
  ...
  public:
    Faculty(string fn, string fc, int sl, string dp, int age, int exp) : facultyName(fn), facultyCode(fc), salary(sl), deptt(dp), age(age), experience(exp) {}
    void display() {
...
class Person : public Student, public Faculty {
  public:
    Person(int sa, string name, int enroll_no, int marks, string facultyName, string facultyCode, int salary, string deptt, int pa, int experience) : Student(sa, name, enroll_no, marks), Faculty(facultyName, facultyCode, salary, deptt, pa, experience) {}
};
  
int main() {
    Person p(20, "John Doe", 12345, 90, "Dr. Jane Doe", "FAC123", 100000, "Computer Science", 40, 15);
    p.Student::display();
    p.Faculty::display();
    return 0;
}
